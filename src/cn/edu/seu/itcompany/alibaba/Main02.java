...
进行优化，优化的规则参照以上，并且优先级依次降低，
合并的时候需要考虑车型（分为17.5m和9.6m两种）：
1、相同车型才能进行合并；
2、两辆同方向的9.6m可以与一辆17.5m的对开车型合并优化 
说明：优化输出结果按照规则分类，例如rule1： 2016120001+2016120002表示将单边车线路ID编码为2016120001和2016120002按照规则1合并优化
 * @author personajian
 *
 */
public class Main02 {
	...
	public static boolean rule1(UnilateralLine bus1,UnilateralLine bus2) {
		boolean rule=false;
		if(bus1.getSCen().equals(bus2.getECen())&&bus1.getECen().equals(bus2.getSCen())) {
			if(bus1.getTType().equals(bus2.getTType()))
				rule=true;
		}
		return rule;
	}

	/*public static boolean rule2(UnilateralLine bus1,UnilateralLine bus2,UnilateralLine bus3) {
		boolean rule=false;
		if()
	}

	public static boolean rule3(UnilateralLine bus1,UnilateralLine bus2,UnilateralLine bus3) {
